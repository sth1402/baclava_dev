#ifndef NORMALIZATIONENGINE_H
#define NORMALIZATIONENGINE_H
...
struct CpIntegrand : public Numer::Func {
  ...
  CpIntegrand(double U_, double t0_, 
              ...
              const compart* psi_,
              const Transform& tr_)
    :  U(U_), t0(t0_), H(H_), P(P_), psi(psi_), tr(tr_) {}
  ...
  inline double psi_val(double tau) const noexcept {
    const double p = comp_param_fast(*psi, tau);
    return p <= 1e-12 ? 1e-12 :
      p >= 1.0 - 1e-12 ? (1.0 - 1e-12) : p;
  }
  ...
  inline double base(double x) const noexcept {
    ...
    double psi_tau = psi_val(tau);
    return fH * (psi_tau + (1.0 - psi_tau) * S);
  }
...
inline Rcpp::NumericVector compute_cp_log_generic(
  ...
  for (std::size_t i = 0; i < keys.size(); ++i) {
    ...
    CpIntegrand f(Uval, t0, &Hc, &Pc, &psic, tr);
